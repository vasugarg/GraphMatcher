...
object NetGraph:
   val logger: Logger = CreateLogger(classOf[NetGraph])
    def load(fileName: String,
             dir: String = outputDirectory,
             isRemoteOriginal: Boolean,
             isRemotePerturbed: Boolean): Option[NetGraph] = {
      val fullPath = if (dir.endsWith("/") || dir.endsWith("\\")) s"$dir$fileName" else s"$dir/$fileName"
      logger.info(s"Loading the NetGraph from $fullPath")
      val inputStream: InputStream = if (isRemoteOriginal || isRemotePerturbed) {
        // Handle remote file
        val conf = new Configuration()
        val fs = FileSystem.get(new URI(fullPath), conf)
      ...
      try {
        ...
        case e: Exception =>
          logger.error(s"Error loading NetGraph from $fullPath", e)
          None
      } finally {
        ois.close()
        inputStream.close()
